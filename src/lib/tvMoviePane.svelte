<script lang="ts">
	...
	interface Props {
		...
		searchResult: IntersectingPeopleOnStarTrek;
	...
	let showConfetti = $state(totalityOfMatchingActors?.length > popularOverlaps[0].actor_overlaps);
	onMount(() => {
		setTimeout(() => {
			showConfetti = false;
		}, 5000);
...
{#if totalityOfMatchingActors?.length > 0}
	<LongListWrapper {isLongList} arrayToIterate={totalityOfMatchingActors} >
		{#snippet children({ matchingActorData })}
